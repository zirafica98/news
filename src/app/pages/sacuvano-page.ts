...
@Component({
  ...
  imports: [RouterLink],
  template: `
    <h1 class="pt-8 text-2xl font-semibold text-white">Sačuvano</h1>
    <p class="mt-1 text-sm text-slate-500">Čuva se samo u ovom browseru.</p>

    @if (!sacuvano.sve().length) {
      <div class="py-16 text-center">
        <p class="text-slate-400">Još ništa nisi sačuvao.</p>
        <p class="mt-1 text-sm text-slate-500">Klikni na ikonicu zastavice pored vesti ili ideje.</p>
      </div>
    } @else {
      <ul class="mt-6 space-y-3">
        @for (s of sacuvano.sve(); track s.kljuc) {
          <li class="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-5">
            ...
            @if (s.url) {
              <a [href]="s.url" target="_blank" rel="noopener" class="mt-3 inline-block text-sm text-amber-400 hover:underline">Otvori ↗</a>
            }
          </li>
        }
      </ul>
